...
def record_request(request):
    ...
    if ((request.data is not None) and (len(request.data) != 0)):
        ...
        if pretty:
            try:
                raw = json.loads(
                    request.data, object_pairs_hook=dict_raise_on_duplicates)
...
class KafkaConsumerThread(threading.Thread):
    def __init__(self):
        ...
        self.is_regex = self.kafka_topic.startswith(
            "^") or self.kafka_topic.startswith("regex:")
        if self.is_regex:
            self.pattern_str = self.kafka_topic.replace(
                "regex:", "") if self.kafka_topic.startswith("regex:") else self.kafka_topic
        else:
    ...
    def run(self):
        ...
        try:
            topics_metadata = self.consumer.list_topics(
                timeout=1).topics.keys()
            if self.is_regex:
                print(
                    f"Subscribing to Kafka topics with regex: {self.kafka_topic}")
                topics_match = [
                    t for t in topics_metadata if re.match(self.pattern_str, t)]
                self.consumer.subscribe(topics_match, on_assign=self.on_assign)
            else:
                print(f"Subscribing to a Kafka topic: {self.kafka_topic}")
                self.consumer.subscribe(
                    [self.kafka_topic], on_assign=self.on_assign)
        ...
        with lock:
                ...
                for header_key, header_value in msg.headers():
                    try:
                        value_str = header_value.decode(
                            'utf-8') if header_value else "(empty)"
                    except UnicodeDecodeError:
            ...
            try:
                ...
                s += json.dumps(payload,
                                indent=4) if pretty else json.dumps(payload)
            except:
